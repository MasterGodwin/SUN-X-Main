        ...
        const cards = document.querySelectorAll('.process-card,.plant-card');
        ...
        cards.forEach((c, i) => { c.style.transitionDelay = (i * 0.07) + 's'; c.style.transition = 'opacity 0.6s cubic-bezier(.34,1.56,.64,1),transform 0.6s cubic-bezier(.34,1.56,.64,1),box-shadow 0.3s,border-color 0.3s'; cardObs.observe(c); });
